...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.decoration_slider', '.no_click', '.decoration_content > div > div', 'after_click'); //увеличивает картинку по тапу в модальном окне калькулятора
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault(); //создает блок, в который помещает оповещение для  пользователя

      var statusMessage = document.createElement('div');
      statusMessage.classList.add('status'); //помешает блок в конец формы

      item.appendChild(statusMessage); //собирает все данные из формы
...
var modals = function modals() {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    close.addEventListener('click', function () {
      ...
      modal.style.display = "none"; //отключает прокрутку страницы под модальным окном
  ...
  bindModal('.popup_engineer_btn', '.popup_engineer', '.popup_engineer .popup_close'); //открытие модального окна при клике на ссылку

  bindModal('.phone_link', '.popup', '.popup .popup_close');
  bindModal('.popup_calc_btn', '.popup_calc', '.popup_calc_close');
  bindModal('.popup_calc_button', '.popup_calc_profile', '.popup_calc_profile_close', false);
  bindModal('.popup_calc_profile_button', '.popup_calc_end', '.popup_calc_end_close', false); //запускает модальное окно после 6 сек
